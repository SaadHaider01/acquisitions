...
const securityMiddleware = async (req, res, next) => {
    try {
        ...
        if (decision.isDenied() && decision.reason.isShield()){
            logger.warn('Shield blocked requests' ,{ ip: req.ip, userAgent: req.get( 'User-Agent'), path: req.path }) ;
        ...
        res.status(500).json({ error: 'Internal Server Error', message: 'SOmething went wrong eith the security middleware' });
    }
};
